...
Each notebook in the `External-Server/` directory exposes a port (usually `8000`) and registers a FastAPI app:
```python
...
1.  **Expose the Model Servers on Colab**:
    *   Open the notebooks in `External-Server/` (`LLM.ipynb`, `classifier.ipynb`, `embedding.ipynb`, `sumarize.ipynb`) on four separate Google Accounts (running on free T4 GPUs).
    *   Execute all cells in each notebook. Copy the generated public URLs (`https://*.trycloudflare.com`).
2.  **Configure API Clients**:
    *   Open `Model/classifier.py`, `Model/embedding.py`, `Model/summarizer.py`, and `Model/qwen.py`.
    *   Paste the respective Cloudflare URL into the `API_URL` variable of each file.
3.  **Run the Pipeline**:
